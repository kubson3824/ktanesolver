...
export default function CryptographySolver({ bomb }: CryptographySolverProps) {
  ...
  const normalizedKeys = useMemo(
    () => keyLetters.map((key) => key.trim().toUpperCase()).filter(Boolean),
    [keyLetters],
  );

  const completedCiphertext = useMemo(() => {
    const trimmed = ciphertext.trim();
    if (!trimmed) return "";
    if (/\s$/.test(ciphertext)) return trimmed;
    const words = trimmed.split(/\s+/);
    return words.length > 1 ? words.slice(0, -1).join(" ") : "";
  }, [ciphertext]);

  const handleSolve = useCallback(async (text: string, background = false) => {
    if (!round?.id || !bomb?.id || !currentModule?.id) {
      if (!background) setError("Missing round, bomb, or module.");
      return;
    }
    if (normalizedKeys.length !== 5) {
      if (!background) setError("Enter exactly 5 key letters (one per key).");
      return;
    }
    if (!text) {
      if (!background) setError("Enter the ciphertext from the module.");
      return;
    }
    ...
    try {
      ...
      if (!background) setError(err instanceof Error ? err.message : "Solve failed.");
    } finally {
  ...
  useEffect(() => {
    if (!completedCiphertext || normalizedKeys.length !== 5 || isSolved || isLoading) return;
    const timeout = window.setTimeout(() => {
      void handleSolve(completedCiphertext, true);
    }, 350);
    return () => window.clearTimeout(timeout);
  }, [completedCiphertext, normalizedKeys, isSolved, isLoading, handleSolve]);
  ...
  return (
    <SolverLayout>
      <SolverSection
        title="Ciphertext"
        description="Type the ciphertext normally; it checks for a unique match after each completed word."
      >
        <textarea
      ...
      <SolverControls
        onSolve={() => handleSolve(ciphertext.trim())}
        onReset={reset}
