...
describe("solver registry", () => {
  ...
  it("registers the Microcontroller solver in the shared registry", () => {
    expect(getLazySolver(ModuleType.MICROCONTROLLER)).not.toBeNull();
  });

  it("registers the Sonic the Hedgehog solver in the shared registry", () => {
    expect(getLazySolver(ModuleType.SONIC_THE_HEDGEHOG)).not.toBeNull();
  });
