...
class SouvenirSolverTest {
	...
	@Test void resolvesBothLabyrinthPortalQuestionFamilies(){BombEntity bomb=new BombEntity();ModuleEntity souvenir=module(ModuleType.SOUVENIR,false,Map.of());List<List<String>>portals=List.of(List.of("A1","B2"),List.of("B2","E6"),List.of("C3","F7"),List.of("D4","A6"),List.of("E5","B7"));ModuleEntity source=module(ModuleType.THE_LABYRINTH,true,Map.of("labyrinthPortals",portals));bomb.setModules(List.of(souvenir,source));assertThat(solve(bomb,souvenir,source.getId(),"portal locations layer 2",List.of(),false).answer()).isEqualTo("B2, E6");assertThat(solve(bomb,souvenir,source.getId(),"portal layers B2",List.of(),false).answer()).isEqualTo("1 (Red), 2 (Orange)");assertThat(solve(bomb,souvenir,source.getId(),"Where was one of the portals in layer 3 (Yellow) in The Labyrinth?",List.of("A2","B4","C3","D6","E2","F5"),false).answer()).isEqualTo("C3");assertThat(solve(bomb,souvenir,source.getId(),"portal layers B2",List.of("1 (Red)","3 (Yellow)","4 (Green)","5 (Blue)"),false).answer()).isEqualTo("1 (Red)");}
	...
	@Test void resolvesFactoryMazeStartRoom(){BombEntity bomb=new BombEntity();ModuleEntity souvenir=module(ModuleType.SOUVENIR,false,Map.of()),source=module(ModuleType.FACTORY_MAZE,true,Map.of("factoryMazeStartRoom","Shipping"));bomb.setModules(List.of(souvenir,source));assertThat(solve(bomb,souvenir,source.getId(),"start room",List.of(),false).answer()).isEqualTo("Shipping");assertThat(solve(bomb,souvenir,source.getId(),"What room did Factory Maze start in?",List.of("Storage","Shipping","Assembly","Cafeteria"),false)).isEqualTo(new SouvenirOutput("Shipping",2));}

	@Test void resolvesBothBrokenGuitarChordsFacts(){BombEntity bomb=new BombEntity();ModuleEntity souvenir=module(ModuleType.SOUVENIR,false,Map.of()),source=module(ModuleType.BROKEN_GUITAR_CHORDS,true,Map.of("brokenGuitarChord","C#m7","brokenGuitarBrokenString",4));bomb.setModules(List.of(souvenir,source));assertThat(solve(bomb,souvenir,source.getId(),"displayed chord",List.of(),false).answer()).isEqualTo("C#m7");assertThat(solve(bomb,souvenir,source.getId(),"broken string",List.of(),false).answer()).isEqualTo("4");assertThat(solve(bomb,souvenir,source.getId(),"What chord was displayed in Broken Guitar Chords?",List.of("Am","C#m7","D7","F"),false).answerIndex()).isEqualTo(2);assertThat(solve(bomb,souvenir,source.getId(),"Which string was broken in Broken Guitar Chords?",List.of("1","2","3","4","5","6"),false).answerIndex()).isEqualTo(4);}

	@Test void resolvesBothRegularCrazyTalkFacts(){BombEntity bomb=new BombEntity();ModuleEntity souvenir=module(ModuleType.SOUVENIR,false,Map.of()),source=module(ModuleType.REGULAR_CRAZY_TALK,true,Map.of("regularCrazyTalkDigit",7,"regularCrazyTalkModifier","Quote: [PHRASE] End quote"));bomb.setModules(List.of(souvenir,source));assertThat(solve(bomb,souvenir,source.getId(),"correct digit",List.of(),false).answer()).isEqualTo("7");assertThat(solve(bomb,souvenir,source.getId(),"embellishment",List.of(),false).answer()).isEqualTo("Quote: [PHRASE] End quote");assertThat(solve(bomb,souvenir,source.getId(),"What digit was displayed by the correct phrase in Regular Crazy Talk?",List.of("3","5","7","9"),false).answerIndex()).isEqualTo(3);assertThat(solve(bomb,souvenir,source.getId(),"What embellishment surrounded the correct phrase in Regular Crazy Talk?",List.of("[PHRASE]","It says: [PHRASE]","Quote: [PHRASE] End quote","“[PHRASE]”"),false).answerIndex()).isEqualTo(3);}

	@Test void resolvesAllFiveSimonSpeaksFacts(){BombEntity bomb=new BombEntity();ModuleEntity souvenir=module(ModuleType.SOUVENIR,false,Map.of()),source=module(ModuleType.SIMON_SPEAKS,true,Map.of("simonSpeaksSouvenir",List.of("top-left","shape 6","Finnish","rouge","yellow")));bomb.setModules(List.of(souvenir,source));List<String>questions=List.of("first flash","second flash","third flash","fourth flash","fifth flash"),expected=List.of("top-left","shape 6","Finnish","rouge","yellow");for(int i=0;i<5;i++){assertThat(solve(bomb,souvenir,source.getId(),questions.get(i),List.of(),false).answer()).isEqualTo(expected.get(i));assertThat(solve(bomb,souvenir,source.getId(),"What was remembered about the "+questions.get(i)+" in Simon Speaks?",List.of("yellow","rouge","shape 6","top-left","Finnish","blue"),false).answer()).isEqualTo(expected.get(i));}}
