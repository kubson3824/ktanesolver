...
public enum ModuleType {
	...
	false), SWITCHES(false), TWO_BITS(false), WORD_SCRAMBLE(false), ANAGRAMS(false), COMBINATION_LOCK(false), ROUND_KEYPAD(false), NUMBER_PAD(false), LISTENING(false), FOREIGN_EXCHANGE_RATES(false), ORIENTATION_CUBE(false), MORSEMATICS(false), CONNECTION_CHECK(false), LETTER_KEYS(false), LOGIC(false), ASTROLOGY(false), MYSTIC_SQUARE(false), CRAZY_TALK(false), ADVENTURE_GAME(false), PLUMBING(false), CRUEL_PIANO_KEYS(false), SAFETY_SAFE(false), CRYPTOGRAPHY(false), CAESAR_CIPHER(false), TURN_THE_KEY(false), TURN_THE_KEYS(false), CHESS(false), MOUSE_IN_THE_MAZE(false), SILLY_SLOTS(false), SKEWED_SLOTS(false), THREE_D_MAZE(false), SIMON_STATES(false), LAUNDRY(false), PROBING(false), ALPHABET(false), MICROCONTROLLER(false), MURDER(false), RESISTORS(false), GAMEPAD(false), TIC_TAC_TOE(false), MONSPLODE_FIGHT(false), SHAPE_SHIFT(false), FOLLOW_THE_LEADER(false), FRIENDSHIP(false), THE_BULB(false), BLIND_ALLEY(false), SEA_SHELLS(false), ENGLISH_TEST(false), ROCK_PAPER_SCISSORS_LIZARD_SPOCK(false);
