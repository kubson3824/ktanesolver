...
public class ColorFlashSolver extends AbstractModuleSolver<ColorFlashInput, ColorFlashOutput> {
	...
	private ColorFlashOutput solveRed(List<ColorFlashEntry> sequence) {
		...
		if(greenWordCount >= 3) {
			int position = findNthOccurrence(sequence, e -> e.word().equalsIgnoreCase("GREEN") || e.color().equalsIgnoreCase("GREEN"), 3);
		...
		int position = findLastOccurrence(sequence, e -> e.word().equalsIgnoreCase("WHITE") || e.color().equalsIgnoreCase("WHITE"));
	...
	private ColorFlashOutput solveYellow(List<ColorFlashEntry> sequence) {
		...
		boolean blueInGreen = sequence.stream().anyMatch(e -> e.word().equalsIgnoreCase("BLUE") && e.color().equalsIgnoreCase("GREEN"));
		if(blueInGreen) {
		...
		boolean whiteInWhiteOrRed = sequence.stream().anyMatch(e -> e.word().equalsIgnoreCase("WHITE") && (e.color().equalsIgnoreCase("WHITE") || e.color().equalsIgnoreCase("RED")));
		if(whiteInWhiteOrRed) {
			int position = findNthOccurrence(sequence, e -> !e.word().equalsIgnoreCase(e.color()), 2);
		...
		int magentaCount = countOccurrences(sequence, e -> e.word().equalsIgnoreCase("MAGENTA") || e.color().equalsIgnoreCase("MAGENTA"));
	...
	private ColorFlashOutput solveGreen(List<ColorFlashEntry> sequence) {
		...
		for(int i = 0; i < sequence.size() - 1; i++) {
			if(sequence.get(i).word().equalsIgnoreCase(sequence.get(i + 1).word()) && !sequence.get(i).color().equalsIgnoreCase(sequence.get(i + 1).color())) {
		...
		if(magentaWordCount >= 3) {
			int position = findFirstOccurrence(sequence, e -> e.word().equalsIgnoreCase("YELLOW") || e.color().equalsIgnoreCase("YELLOW"));
	...
	private ColorFlashOutput solveBlue(List<ColorFlashEntry> sequence) {
		...
		boolean redInYellow = sequence.stream().anyMatch(e -> e.word().equalsIgnoreCase("RED") && e.color().equalsIgnoreCase("YELLOW"));
		boolean yellowInWhite = sequence.stream().anyMatch(e -> e.word().equalsIgnoreCase("YELLOW") && e.color().equalsIgnoreCase("WHITE"));
		if(redInYellow || yellowInWhite) {
			int position = findFirstOccurrence(sequence, e -> e.word().equalsIgnoreCase("WHITE") && e.color().equalsIgnoreCase("RED"));
		...
		int position = findLastOccurrence(sequence, e -> e.word().equalsIgnoreCase("GREEN") || e.color().equalsIgnoreCase("GREEN"));
	...
	private ColorFlashOutput solveMagenta(List<ColorFlashEntry> sequence) {
		...
		for(int i = 0; i < sequence.size() - 1; i++) {
			if(sequence.get(i).color().equalsIgnoreCase(sequence.get(i + 1).color()) && !sequence.get(i).word().equalsIgnoreCase(sequence.get(i + 1).word())) {
	...
	private ColorFlashOutput solveWhite(List<ColorFlashEntry> sequence) {
		...
		if(thirdWord.color().equalsIgnoreCase(fourthWord.word()) || thirdWord.color().equalsIgnoreCase(fifthWord.word())) {
			int position = findFirstOccurrence(sequence, e -> e.word().equalsIgnoreCase("BLUE") || e.color().equalsIgnoreCase("BLUE"));
		...
		boolean yellowInRed = sequence.stream().anyMatch(e -> e.word().equalsIgnoreCase("YELLOW") && e.color().equalsIgnoreCase("RED"));
		if(yellowInRed) {
