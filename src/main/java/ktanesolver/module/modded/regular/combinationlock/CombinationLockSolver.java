...
@Service
@ModuleInfo (type = ModuleType.COMBINATION_LOCK, id = "combination_lock", name = "Combination Lock", category = ModuleCatalogDto.ModuleCategory.MODDED_REGULAR, description = "Turn the dial: RIGHT to X → LEFT to Y → RIGHT to Z", tags = {
	"modded", "regular", "combination"})
public class CombinationLockSolver extends AbstractModuleSolver<CombinationLockInput, CombinationLockOutput> {
	...
	public SolveResult<CombinationLockOutput> doSolve(RoundEntity round, BombEntity bomb, ModuleEntity module, CombinationLockInput input) {
		...
		int solvedModulesCount = (int)bomb.getModules().stream().filter(ModuleEntity::isSolved).count();
		...
		try {
			...
			String instruction = String.format("Turn the dial: RIGHT to %d → LEFT to %d → RIGHT to %d", firstNumber, secondNumber, thirdNumber);
