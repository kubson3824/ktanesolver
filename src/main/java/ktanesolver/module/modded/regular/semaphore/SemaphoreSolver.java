...
@Service
@ModuleInfo (type = ModuleType.SEMAPHORE, id = "semaphore", name = "Semaphore", category = ModuleCatalogDto.ModuleCategory.MODDED_REGULAR, description = "Decode the semaphore flags and form words", tags = {
	"decoding", "pattern"})
public class SemaphoreSolver extends AbstractModuleSolver<SemaphoreInput, SemaphoreOutput> {
	...
	public SolveResult<SemaphoreOutput> doSolve(RoundEntity round, BombEntity bomb, ModuleEntity module, SemaphoreInput input) {
		...
		for(FlagAngles angles: flagAnglesSequence) {
			...
			if(c >= 'A' && c <= 'Z' || c >= '0' && c <= '9') {
				sequenceCharCounts.put(c, sequenceCharCounts.getOrDefault(c, 0) + 1);
			}
		}
		...
		for(char c: serialNumber.toUpperCase().toCharArray()) {
			if((c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9')) {
				serialCharCounts.put(c, serialCharCounts.getOrDefault(c, 0) + 1);
			}
		}
