    ...
    private SimonColor mapColor(SimonColor flashed, boolean hasVowel, int strikes) {
        ...
        if (hasVowel) {
            if (flashed == SimonColor.RED) {
                if (s == 0) return SimonColor.BLUE;
                if (s == 1) return SimonColor.YELLOW;
                return SimonColor.GREEN;
            } else if (flashed == SimonColor.BLUE) {
                if (s == 0) return SimonColor.RED;
                if (s == 1) return SimonColor.GREEN;
                return SimonColor.RED;
            } else if (flashed == SimonColor.GREEN) {
                if (s == 0) return SimonColor.YELLOW;
                if (s == 1) return SimonColor.BLUE;
                return SimonColor.YELLOW;
            } else { // YELLOW
                if (s == 0) return SimonColor.GREEN;
                if (s == 1) return SimonColor.RED;
